// src/Pages/Supervisor/Inward/Inward.jsx
...
const Inward = () => {
  ...
  const generateChallanNo = () => {
    ...
    return `IN${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}${timestamp}`;
  };

  const fetchChallans = async () => {
    try {
      const response = await fetch('/api/inward-challans', {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.uploadedPhoto) {
      setSubmitStatus('❌ Please upload delivery photo');
      setTimeout(() => setSubmitStatus(''), 3000);
      return;
    }

    setIsSubmitting(true);
    setSubmitStatus('💾 Processing inward entry...');
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitStatus('✅ Inward entry successful! PDF generated.');
        fetchChallans();
        setTimeout(() => setSubmitStatus(''), 3000);
      } else {
        throw new Error('Failed to process inward');
      }

    } catch (error) {
      setSubmitStatus('❌ Failed to process inward. Please try again.');
      setTimeout(() => setSubmitStatus(''), 3000);
    } finally {
  ...
  const viewPDF = (challanId) => {
    window.open(`/api/inward-challans/${challanId}/pdf`, '_blank');
  };
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Paper elevation={3} sx={{ mb: 4 }}>
        <Tabs value={activeTab} onChange={(e, v) => setActiveTab(v)}>
          <Tab icon={<InwardIcon />} iconPosition="start" label="Material Inward" />
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ border: '1px solid #ccc', borderRadius: 1, p: 2, minHeight: '120px' }}>
                  <FormControlLabel
                    ...
                    label="Material quantity and quality is correct"
                  />
                  <TextField
                    name="remarks"
